...
public class QuickerThanMergeSort implements Sort {
    ...
    public void sort(double[] arr) {
        ...
        while (sortedPortions.size() > 1) {
            int firstStart = sortedPortions.get(0)[0]; // starting index of first sorted sub-array
            int firstEnd = sortedPortions.get(0)[1]; // end index of first sorted sub-array
            int secondStart = sortedPortions.get(1)[0]; // starting index of second sorted sub-array
            int secondEnd = sortedPortions.get(1)[1]; // end index of second sorted sub-array

            merge(arr, Arrays.copyOfRange(arr, firstStart, firstEnd + 1), Arrays.copyOfRange(arr, secondStart,  secondEnd + 1));
            ...
            sortedPortions.remove(2); //remove 2 and 1 since length is now + 1 since we added at pos 0
